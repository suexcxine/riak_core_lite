...
standard_join(Node, Ring, Rejoin, Auto) ->
	...
	_ ->
	    ...
	    Ring4 = riak_core_ring:update_member_meta(node(),
						      Ring3,
						      node(),
						      gossip_vsn,
						      2),
...
register(App, [{bucket_fixup, FixupMod} | T]) ->
    register_mod(get_app(App, FixupMod),
		 FixupMod,
		 bucket_fixups),
    register(App, T);
...
register(App, [{health_check, HealthMFA} | T]) ->
    register_metadata(get_app(App, HealthMFA),
		      HealthMFA,
		      health_checks),
    register(App, T);
register(App,
	 [{bucket_validator, ValidationMod} | T]) ->
    register_mod(get_app(App, ValidationMod),
		 ValidationMod,
		 bucket_validators),
    register(App, T);
