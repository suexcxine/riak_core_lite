...
index_to_int(Index) ->
    <<IndexAsInt:160/integer>> = Index, IndexAsInt.
...
int_to_index(Int) -> binary_to_integer(Int).
...
offsets({NumPartitions, Nodes}) ->
    [(?RINGTOP) div NumPartitions || _ <- Nodes].
...
inverse_pred_test() ->
    ...
    S = [I
         || {I, _}
                <- chash_legacy:successors(chash_legacy:key_of(4),
                                           CHash)],
    P = [I
         || {I, _}
                <- chash_legacy:predecessors(chash_legacy:key_of(4),
                                             CHash)],
