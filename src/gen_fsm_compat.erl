...
-callback terminate(Reason :: normal |
                              shutdown |
                              {shutdown, term()} |
                              term(),
...
send_event({via, Module, Name}, Event) ->
    catch Module:send(Name, {'$gen_event', Event}),
    ok;
...
system_code_change([Name,
    ...
    case catch Module:code_change(OldVsn,
                                  StateName,
                                  StateData,
                                  Extra)
        of
...
dispatch({'$gen_sync_all_state_event', From, Event},
    ...
    Module:handle_sync_event(Event,
                             From,
                             StateName,
                             StateData);
...
terminate(Reason, Name, Msg, Module, StateName,
    ...
    case erlang:function_exported(Module, terminate, 3) of
        true ->
            case catch Module:terminate(Reason,
                                        StateName,
                                        StateData)
                of
                {'EXIT', R} ->
                    FmtStateData = format_status(terminate,
                                                 Module,
                                                 get(),
                                                 StateData),
                    error_info(R,
    ...
    case Reason of
        ...
        _ ->
            FmtStateData1 = format_status(terminate,
                                          Module,
                                          get(),
                                          StateData),
            error_info(Reason,
