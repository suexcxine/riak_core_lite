...
call(Name, Msg) ->
    call_reply(catch gen:call(Name,
			      '$vnode_proxy_call',
			      Msg)).
...
handle_overload(Msg,
	...
	{'$gen_event',
	 #riak_vnode_req_v1{sender = Sender,
			    request = Request}} ->
	    catch Mod:handle_overload_command(Request,
					      Sender,
					      Index);
	{'$gen_all_state_event',
	 #riak_vnode_req_v1{sender = Sender,
    ...
    {timeout,
     ...
     {foreach,
      fun () ->
	      ...
	      meck:expect(riak_core_vnode_manager,
			  get_vnode_pid,
       ...
       fun ({VnodePid, ProxyPid}) ->
		...
		fun () ->
			...
			receive
				...
				?assertEqual(((?DEFAULT_OVERLOAD_THRESHOLD) +
						  10),
					     Count)
			end
		end}
