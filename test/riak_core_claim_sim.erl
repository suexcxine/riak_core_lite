...
help() ->
    io:format("\nClaim Simulator - simulates effect "
              "of join/leave commands\n\n  riak_core_claim_s"
              "im:run([Opts]).\n\nOpts = {claimant, "
              "node()} | \n       {ringfile, path()} "
              "| \n       {ring, ring()} | {appvars, "
              "[{Key,Val}]} \n\n       {wants, {WMod,WFun}}\n "
              "      {choose, {CMod,CFun} | {CMod,CFun,CPara"
              "m}}\n       {target_n_val, int()}\n "
              "      {prepare, finish | cancel}\n  "
              "     {print, false | standard_io | io_device("
              ")}\n       {cmds, [Commands]}\n\nCommands "
              "= {join, node()} | {join, integer()}} "
              "| {leave, node()} | [Commands]\n\nThe "
              "simulator uses the options to select "
              "the ring to use\n  if claimant is set, "
              "grab the ring and target n-val from "
              "that node\n  if ringfile is set, load "
              "the ring file from disk\n  if neither "
              "set, use the current ring from the node\n\nAn"
              "y active transfers are handled with "
              "the prepare option.  The default is "
              "to \nfinish the transfers, but they "
              "can be cancelled with {prepare, cancel}\n\nTh"
              "e cmds list provides the cluster transitions "
              "- commands can either be individual\njoin/lea"
              "ve commands or a batch to be added all "
              "at once.\n{cmds, [[{join, 'riak@hosta'},{join"
              ", 'riak@hostb'}],{join,'riak@hostc'}]}\nwould "
              "join hosta/hostb and recalculate claim, "
              "then join hostc.\n\nCaveat: To use leave "
              "in commands, target_n_val must be set "
              "in the app env.\n\nExamples:\n  Simulate "
              "adding 9 nodes in a single batch to "
              "the current ring\n     riak_core_claim_sim:ru"
              "n([{cmds, [[{join, 9}]]}]).\n\n  Simulate "
              "cancelling transfers, adding a node "
              "then and removing a different node\n "
              "    riak_core_claim_sim:run([{cmds, "
              "[{join, 'riak@newnode'},\n          "
              "                            {leave, "
              "'riak@oldnode'}]},\n                "
              "              {prepare, cancel}]).\n\n").
...
run([Config]) when is_list(Config) ->
    ...
    case file:consult(Config) of
        {ok, Terms} -> run(Terms);
        {error, Reason} -> {cannot_read_config, Reason}
    end;
run(Opts) ->
    ...
    Wants = proplists:get_value(wants,
                                Opts,
    ...
    ReturnRing = proplists:get_value(return_ring,
                                     Opts,
                                     false),
    TN0 = proplists:get_value(target_n_val,
                              Opts,
                              application:get_env(riak_core,
                                                  target_n_val,
                                                  undefined)),
    {Ring, TN} = case {Claimant, RingFile, RingArg} of
                     {undefined, undefined, undefined} ->
                         {ok, Ring0} = riak_core_ring_manager:get_raw_ring(),
                         {Ring0, TN0};
                     {Claimant, undefined, undefined} ->
                         {ok, Ring0} = rpc:call(Claimant,
                                                riak_core_ring_manager,
                                                get_raw_ring,
                                                []),
                         ClaimantTN = rpc:call(Claimant,
                                               application,
                                               get_env,
                                               [riak_core,
                                                target_n_val,
                     ...
                     {undefined, RingFile, undefined} ->
                         ...
                         {Ring0, TN0};
                     {undefined, undefined, RingArg} -> {RingArg, TN0};
                     _ ->
                         help(),
                         throw({bad_opts, Opts})
                 end,
    ...
    SimOpts = SimOpts0#simopts{wants = Wants,
                               choose = Choose1, prepare = Prepare,
                               return_ring = ReturnRing},
    dryrun(Ring, Cmds, SimOpts).

add_choose_params(Choose, TN) ->
    Params = case Choose of
                 {CMod, CFun} -> [];
                 {CMod, CFun, Params0} -> Params0
             end,
    Params1 = [{target_n_val, TN}
               | proplists:delete(target_n_val, Params)],
    {CMod, CFun, Params1}.
...
read_ringfile(RingFile) ->
    case file:read_file(RingFile) of
        ...
        {error, Reason} -> throw({bad_ring, Reason})
    end.

setup_environment(Vars) ->
    _ = [application:set_env(riak_core, Key, Val)
         || {Key, Val} <- Vars],
    ok.
...
make_rebalance(false, _TN) -> fun (_R1, _R2) -> ok end;
    ...
    fun (Ring, Ring2) ->
            ...
            Next = [{Idx, PrevOwner, NewOwner}
                    || {{Idx, PrevOwner}, {Idx, NewOwner}} <- Owners3,
                       PrevOwner /= NewOwner],
            Tally = lists:foldl(fun ({_, PrevOwner, NewOwner},
                                     Tally) ->
                                        dict:update_counter({PrevOwner,
                                                             NewOwner},
                                                            1,
                                                            Tally)
                                end,
            ...
            o(IoDev,
              "Preflists violating targetN=~p: ~p~n",
              [TN, length(BadPL)]),
            _ = [o(IoDev,
                   "~b transfers from ~p to ~p~n",
                   [Count, PrevOwner, NewOwner])
                 || {{PrevOwner, NewOwner}, Count}
                        <- dict:to_list(Tally)],
            ok
    end.
...
make_analysis(IoDev, Analysis) ->
    fun (Ring, _Cmds) -> run_analysis(IoDev, Analysis, Ring)
    end.
...
dryrun1(Ring00, CmdsL,
        #simopts{wants = Wants, choose = Choose,
                 prepare = Prepare, current = Current,
                 prepared = Prepared, percmd = PerCmd,
                 postxfer = PostXfer, rebalance = Rebalance,
                 analysis = Analysis}) ->
    Current(Ring00),
    Ring01 = case Prepare of
                 cancel -> riak_core_ring:cancel_transfers(Ring00);
                 finish -> riak_core_ring:future_ring(Ring00)
             end,
    Prepared(Ring01, Prepare),
    Ring0 = run_rebalance(Ring01, Wants, Choose, Rebalance),
    lists:foldl(fun (Cmds, RingAcc1) ->
                        NewRing = lists:foldl(fun (Cmd, RingAcc2) ->
                                                      RingAcc3 = command(Cmd,
                                                                         RingAcc2),
                                                      PerCmd(RingAcc3, Cmd),
                                                      RingAcc3
                                              end,
                                              ...
                                              Cmds),
                        {_, NewRing2} =
                            riak_core_claimant:reassign_indices(NewRing),
...
sim_node(N) ->
    list_to_atom(lists:flatten(io_lib:format("sim~b@127.0.0.1",
                                             [N]))).
...
pretty_print(false, _Ring, _TN) -> ok;
pretty_print(IoDev, Ring, TN) ->
    riak_core_ring:pretty_print(Ring,
                                [legend, {out, IoDev}, {target_n, TN}]).

%% Run analysis of the ring
run_analysis(false, _Analysis, _Ring) -> ok;
run_analysis(IoDev, Analysis, Ring) ->
    case proplists:get_value(failures, Analysis, 0) of
        0 -> ok;
        Failures ->
            o(IoDev,
              "Failure analysis - up to ~p nodes\n",
              [Failures]),
            NVal = proplists:get_value(n_val, Analysis, 3),
            try riak_core_claim_util:print_analysis(IoDev,
                                                    riak_core_claim_util:sort_by_down_fbmax(riak_core_claim_util:node_sensitivity(Ring,
                                                                                                                                  NVal,
                                                                                                                                  Failures)))
            catch
                _:Err ->
...
o(false, _Fmt) -> ok;
o(IoDev, Fmt) -> io:format(IoDev, Fmt).

o(false, _Fmt, _Args) -> ok;
o(IoDev, Fmt, Args) -> io:format(IoDev, Fmt, Args).
...
commission(Base, Test, Claims) when is_list(Claims) ->
    [try commission(Base, Test, Claim) catch
         _:Err ->
...
commission(Base, Test, {Wants, Choose}) ->
    ...
    SeqJoinCmds = [[{join, sim_node(I)}]
                   || I <- lists:seq(2, Nodes)],
    BulkJoinCmds = [[{join, sim_node(I)}
                     || I <- lists:seq(2, Nodes)]],
    Dir = commission_test_dir(Base,
                              RingSize,
                              Nodes,
                              NVal,
                              TN,
                          ...
                          analysis =
                              fun (Ring2, Cmds) ->
                                      ...
                                      ok = file:write_file(FN,
                                                           term_to_binary(Ring2)),
                                      Stats = try
                                                  riak_core_claim_util:ring_stats(Ring2,
                                                                                  TN)
                                              catch
                                                  _:Reason ->
                                      ...
                                      io:format(SeqFh,
                                                "\"~w\",~p,~p,~p\n",
                                                [Cmds,
                                                 proplists:get_value(balance,
                                                                     Stats,
                                                                     undefined),
                                                 proplists:get_value(violations,
                                                                     Stats,
                                                                     undefined),
                                                 proplists:get_value(diversity,
                                                                     Stats,
                                                                     undefined)])
                              end},
    put(sim_seq, 2),
    ...
    {ok, BulkFh} = file:open(filename:join([Dir,
                                            "bulk.txt"]),
                             [write]),
    io:format(BulkFh,
              "cmds,balance,violations,diversity\n",
              []),
    BulkSimOpts = #simopts{wants = Wants, choose = Choose,
                           analysis =
                               fun (Ring2, Cmds) ->
                                       Seq = get(sim_seq),
                                       put(sim_seq, Seq + 1),
...
commission_test_dir(Base, RingSize, Nodes, NVal, TN,
                    {_Mod, Choose}) ->
    filename:join(Base,
    ...
    [[{n_val, NVal},
     ...
     || Q
            <- [64,
                ...
                8192,
                16384],
        NVal <- lists:seq(1, 9), TNDelta <- lists:seq(0, 3)].

commission_claims() ->
    [{{riak_core_claim, wants_claim_v1},
      {riak_core_claim, choose_claim_v1}},
    ...
    {timeout,
     ...
     ?_test(begin
                ...
                [begin
                     ...
                     ?assert((filelib:is_file(filename:join(TD,
                                                            "sequential.txt")))),
                     ?assert((filelib:is_file(filename:join(TD,
                                                            "sj5.ring")))),
                     ?assert((filelib:is_file(filename:join(TD,
