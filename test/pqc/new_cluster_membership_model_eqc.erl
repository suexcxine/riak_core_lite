...
-record(chstate_v2,
        {nodename ::
             node(),          % the Node responsible for this chstate
         vclock ::
             vclock:vclock(), % for this chstate object, entries are
                              % {Node, Ctr}
         chring ::
             chash:chash(),   % chash ring of {IndexAsInt, Node} mappings
         meta ::
             dict:dict(),          % dict of cluster-wide other data (primarily
         % bucket N-value, etc)
         clustername :: {node(), term()},
         next ::
             [{integer(), node(), node(), [module()],
               awaiting | complete}],
         members ::
             [{node(), {member_status(), vclock:vclock(), []}}],
         claimant :: node(),
         seen :: [{node(), vclock:vclock()}],
         rvsn :: vclock:vclock()}).

-type member_status() :: valid |
                         invalid |
                         leaving |
                         exiting.
...
%% Node state
-record(nstate,
        {chstate = #chstate_v2{} :: #chstate_v2{},
         partitions = [] ::
             orddict:orddict(atom(), ordsets:ordsets(integer())),
         has_data = [] ::
             orddict:orddict(atom(), ordsets:ordsets(integer())),
         joined_to :: integer()}).
     ...
     [{setup,
       ...
       [{inorder,
         [manual_test_list(),
          ...
          {timeout,
           60000000, % timeout is in msec
           ?_assertEqual(true,
                         (catch proper:quickcheck(numtests(?TEST_ITERATIONS,
                                                           ?QC_OUT((prop_join()))))))}]}]}]}.
...
prop_join() ->
    ?FORALL(Cmds, (commands(?MODULE)),
            (?TRAPEXIT(begin
                           {_History, State, Result} = run_commands(?MODULE,
                                                                    Cmds),
                           test_ring_convergence(State),
                           aggregate(command_names(Cmds),
                                     ?WHENFAIL((?debugFmt("~n~p~n", [Result])),
                                               (Result == ok)))
                       end))).
...
handoff_all_partitions(Node, State) ->
    case dict:find(Node, State#state.nstates) of
        ...
        {ok, NState} ->
            ...
            lists:foldl(fun (Mod, State0) ->
                                Partitions = orddict:fetch(Mod, AllPartitions),
                                lists:foldl(fun (Idx, State00) ->
                                                    do_maybe(State00,
                                                             maybe_handoff,
                                                             [Mod, Node, Idx])
                                            end,
...
handoff_all(State) ->
    ...
    State2 = lists:foldl(fun handoff_all_partitions/2,
                         State,
                         Members),
...
do_maybe(State, Cmd, Args) ->
    case precondition(State, {call, ?MODULE, Cmd, Args}) of
        true -> run(State, {call, ?MODULE, Cmd, Args});
        false -> State
    end.
...
test_ring_convergence(State) ->
    ...
    case Nodes of
        ...
        _ ->
            ...
            {Result, State3} = do_converge_ring(State2,
                                                {1000, true}),
            ?assert(Result),
            State3
    end.
...
do_gossip(State, N2, N1) ->
    case precondition(State,
                      {call, ?MODULE, random_gossip, [N2, N1]})
        of
        true ->
            {true,
             run(State, {call, ?MODULE, random_gossip, [N2, N1]})};
        false -> {false, State}
    end.

do_converge_ring(State, {_, false}) -> {true, State};
do_converge_ring(State, {0, _}) -> {false, State};
do_converge_ring(State, {RC, true}) ->
    %%Nodes = get_members(State#state.members, [valid, leaving]),
    Nodes = State#state.primary,
    {_, Changed1, State2} = lists:foldl(fun (N2,
                                             {N1, Changed0, State0}) ->
                                                case do_gossip(State0, N2, N1)
                                                    of
                                                    {true, S} ->
                                                        Changed = not
                                                                      equal_cstate(get_cstate(State0,
                                                                                              N2),
                                                                                   get_cstate(S,
                                                                                              N2)),
                                                        {N2,
    ...
    {Changed2, State3} = lists:foldl(fun (N2,
                                          {Changed0, State0}) ->
...
make_default_ring(Size, Nodes) ->
    Indices = [Idx
               || {Idx, _}
                      <- chash:nodes(chash:fresh(Size, undefined))],
    N = erlang:length(Nodes),
    [{lists:nth(Idx + 1, Indices),
      lists:nth(Idx rem N + 1, Nodes)}
     || Idx <- lists:seq(0, Size - 1)].

vnode_modules(_State, _Node) -> [riak_kv, riak_pipe].

all_owners(#chstate_v2{chring = Ring}) ->
    chash:nodes(Ring).

all_members(#chstate_v2{members = Members}) ->
    get_members(Members).

claiming_members(#chstate_v2{members = Members}) ->
    get_members(Members, [joining, valid, down]).

gossip_members(#chstate_v2{members = Members}) ->
    get_members(Members,
...
owner(#chstate_v2{chring = Ring}, Idx) ->
    chash:lookup(Idx, Ring).

owner_node(CState) -> CState#chstate_v2.nodename.

init_node_state(State, Node) ->
    Ring = chash:fresh(State#state.ring_size, Node),
    Indices = indices(#chstate_v2{chring = Ring}, Node),
    RVsn = vclock:increment(Node, vclock:fresh()),
    VClock = vclock:increment(Node, vclock:fresh()),
    CState = #chstate_v2{nodename = Node,
                         clustername = {Node, erlang:now()},
                         members = [{Node, {valid, VClock, []}}], chring = Ring,
                         next = [], claimant = Node, seen = [{Node, VClock}],
                         rvsn = RVsn, vclock = VClock},
...
init_node_state(State, Node, CState) ->
    ...
    #nstate{chstate = CState#chstate_v2{nodename = Node},
            partitions = VNodes, has_data = VNodes}.

%% EQC state_m callbacks
initial_state() ->
    #state{primary = [], others = [],
           rejoin = orddict:new(), removed = [], down = [],
           ring_size = ?RING, members = [], allowed = [],
           active_handoffs = [], seed = {3172, 9814, 20125},
           split = dict:new()}.
...
g_idx(State) ->
    Indices = [Idx
               || {Idx, _}
                      <- chash:nodes(chash:fresh(State#state.ring_size,
                                                 undefined))],
    elements(Indices).
...
g_posint() -> ?SUCHTHAT(X, (largeint()), (X > 0)).

g_seed() -> {g_posint(), g_posint(), g_posint()}.

g_handoff(State, Nodes) ->
    ?LET(Node, (elements(Nodes)),
         [elements(vnode_modules(State, Node)),
          Node,
          g_idx(State)]).
...
command(State = #state{primary = []}) ->
    {call,
     ?MODULE,
     initial_cluster,
     [g_initial_nodes(), g_random_ring(State), g_seed()]};
command(State = #state{primary = Primary,
                       others = Others, active_handoffs = Active}) ->
    oneof([{call,
            ?MODULE,
            read,
            [elements(Primary), g_idx(State)]},
           {call,
            ?MODULE,
            random_gossip,
           ...
           {call, ?MODULE, leave, [elements(Primary)]}]
              ...
              [{call,
                ?MODULE,
                join,
                [elements(Others), elements(Primary)]}
               || Others /= []]
                  ++
                  [{call, ?MODULE, finish_handoff, [elements(Active)]}
                   || Active /= []]).
                ...
                lists:member(Node, State#state.primary) andalso
                    lists:member(PNode, State#state.primary) andalso
                        dict:is_key(Node, State#state.nstates) andalso
                            not is_joining(PNode, State);
precondition2(State,
              {call, _, random_gossip, [Node1, Node2]}) ->
...
precondition2(State, _) -> State#state.primary /= [].
...
maybe_inconsistent(State, Node) ->
    ...
    case {JoinedTo, JTStatus} of
        {undefined, _} -> false;
        {_, {invalid, _, _}} ->
            ClaimantCS = get_cstate(State,
                                    CState#chstate_v2.claimant),
            not
                lists:member(Node,
                             get_members(ClaimantCS#chstate_v2.members));
        _ ->
            JoinCS = get_cstate(State, JoinedTo),
            not
                lists:member(Node,
                             get_members(JoinCS#chstate_v2.members))
    end.
...
next_state(State, Result, Call) ->
    ...
    try r_next_state2(State, Result, Call) of
        ...
        _:_ -> save_random(State#state{old_seed = OldSeed})
    end.
...
next_state2(State, _, {call, _, read, _}) -> State;
next_state2(State, _Result,
            {call,
             _,
             initial_cluster,
             [{Members, Others}, RRing, Seed]}) ->
    s_initial_cluster(State#state{seed = Seed},
                      Members,
                      Others,
                      RRing);
next_state2(State, _Result,
            {call, _, random_gossip, [Node, OtherNode]}) ->
    NState = get_nstate(State, Node),
    OtherCS = get_cstate(State, OtherNode),
    s_random_gossip(State,
                    Node,
                    NState,
                    OtherNode,
                    OtherCS);
...
next_state2(State, _Result,
            {call, _, finish_handoff, [AH]}) ->
    s_finish_handoff(State, AH).

postcondition(State, {call, _, read, [Node, Idx]},
              _Result) ->
    CState = get_cstate(State, Node),
...
check_transfers(#nstate{chstate = CState}) ->
    L = [Idx
         || {Idx, Owner, NextOwner, _, _}
                <- CState#chstate_v2.next,
            Owner =:= NextOwner],
    L =:= [].

check_rvsn(State) ->
    dict:map(fun (_, NState) ->
                     RVsn = (NState#nstate.chstate)#chstate_v2.rvsn,
...
check_sorted_members(State) ->
    L1 = [begin
              M = (NS#nstate.chstate)#chstate_v2.members,
              M /= [] andalso M == lists:ukeysort(1, M)
          end
...
%% Dummy commands
initial_cluster(_, _, _) -> ok.

read(_, _) -> ok.

maybe_handoff(_, _, _) -> ok.

finish_handoff({_Mod, _Idx, _Prev, _New}) -> ok.
...
s_initial_cluster(State, Members, Others, RandomRing) ->
    ...
    Members3 = [{M,
                 {valid, vclock:increment(M, vclock:fresh()), []}}
                || M <- Members2],
    RVsn = vclock:increment(Claimant, vclock:fresh()),
    VClock = vclock:increment(Claimant, vclock:fresh()),
    Seen = [{M, VClock} || M <- Members2],
    CState = #chstate_v2{clustername =
                             {Claimant, erlang:now()},
                         members = Members3, chring = Ring, next = [],
                         claimant = Claimant, seen = Seen, rvsn = RVsn,
                         vclock = VClock},
    SMembers = [{Node, init_node_state(State, Node, CState)}
                || Node <- Members2],
    SOthers = [{Node, init_node_state(State, Node)}
               || Node <- Others],
    NStates = dict:from_list(SMembers ++ SOthers),
    State#state{members = Members3, primary = Members2,
                others = Others, allowed = [], random_ring = RandomRing,
                nstates = NStates}.

s_random_gossip(State, Node, NState, OtherNode,
                OtherCS) ->
    State2 = s_gossip(State,
                      {Node, NState},
                      OtherNode,
                      OtherCS),
    State2.
...
s_gossip(State, {Node, NState}, OtherNode, OtherCS) ->
    ...
    WrongCluster = CState#chstate_v2.clustername /=
                       OtherCS#chstate_v2.clustername,
    ...
    case {WrongCluster, OtherStatus, Changed} of
        ...
        {_, invalid, _} ->
            ...
            OtherRemoved = lists:member(OtherNode,
                                        State#state.removed),
                ...
                _ ->
                    case orddict:find(OtherNode, State#state.rejoin) of
                        ...
                        _ ->
                            ...
                            ?debugFmt("S(~p): ~p~n",
                                      [OtherNode,
                                       member_status(OtherCS, OtherNode)]),
...
s_join(State, Node, NState, PNode) ->
    ...
    case orddict:find(Node, JoinCS#chstate_v2.members) of
        %% Don't allow a node to rejoin while it's still exiting.
        {ok, {exiting, _, _}} -> State;
        _ ->
            ...
            case orddict:is_key(Node, State#state.rejoin) of
                ...
                false -> Rejoin2 = State#state.rejoin
            end,
            JoinCS2 = add_member(Node, JoinCS, Node),
            %% ?debugFmt("J: ~p~n", [JoinCS2#chstate_v2.members]),
            JoinCS3 = JoinCS2#chstate_v2{nodename = Node},
            State2 = update_cstate(State, PNode, JoinCS2),
            Partitions = start_vnodes(State, Node, []),
            State3 = update_nstate(State2,
                                   Node,
                                   NState#nstate{chstate = JoinCS3,
                                                 partitions = Partitions,
                                                 has_data = [],
                                                 joined_to = PNode}),
            State3#state{primary = Primary2, others = Others2,
                         rejoin = Rejoin2}
    end.
...
s_down(State, Node, PNode) ->
    ...
    case orddict:find(Node, DownCS#chstate_v2.members) of
        {ok, {valid, _, _}} ->
...
s_remove(State, Node, PNode, Shutdown) ->
    ...
    case orddict:find(Node, PNodeCS#chstate_v2.members) of
        {ok, {invalid, _, _}} -> State;
        {ok, {_, _, _}} ->
            ...
            Allowed2 = State#state.allowed ++
                           indices(NodeCS, Node) ++ PendingIdx,
            case Shutdown of
                true ->
            ...
            State2#state{primary = Primary2, allowed = Allowed2,
                         nstates = NStates2, removed = Removed2};
        _ -> State
    end.
...
set_member(Node, CState, Member, Status) ->
    VClock = vclock:increment(Node,
                              CState#chstate_v2.vclock),
    CState2 = set_member(Node,
                         CState,
                         Member,
                         Status,
                         same_vclock),
    CState2#chstate_v2{vclock = VClock}.
...
set_member(Node, CState, Member, Status, same_vclock) ->
    Members2 = orddict:update(Member,
                              ...
                              {Status,
                               ...
                               []},
                              CState#chstate_v2.members),
    CState#chstate_v2{members = Members2}.

handle_cast(State = #state{random_ring = RRing,
                           others = Others, nstates = NStates},
            _, Node, shutdown) ->
    %% Occasionally allow nodes to re-join
    RandomList = lists:nthtail(erlang:length(RRing) div 2,
                               RRing),
    case lists:member(Node, RandomList) of
        ...
        false ->
            ...
            Rejoin2 = orddict:store(Node,
                                    false,
                                    State#state.rejoin),
            NStates2 = NStates
    end,
    Primary2 = State#state.primary -- [Node],
    State#state{nstates = NStates2, primary = Primary2,
                others = Others2, rejoin = Rejoin2};
handle_cast(State, _, Node, {rejoin, OtherCS}) ->
    CState = get_cstate(State, Node),
    SameCluster = CState#chstate_v2.clustername =:=
                      OtherCS#chstate_v2.clustername,
    case SameCluster of
        true ->
...
handle_cast(State, _, Node,
            {remove_member, OtherNode}) ->
    CState = get_cstate(State, Node),
    CState2 = remove_member(Node, CState, OtherNode),
    update_cstate(State, Node, CState2);
handle_cast(State, OtherNode, Node,
            {gossip, OtherCS}) ->
    NState = get_nstate(State, Node),
    State2 = s_gossip(State,
                      {Node, NState},
                      ...
                      OtherCS),
    State2;
handle_cast(State, _, _, _) -> State.

handle_call(State, _Sender, _Receiver, _Msg) ->
    {ok, State}.

call(State, undefined, _, _) -> {fail, State};
call(State, _Node, undefined, _) -> {fail, State};
call(State, Sender, Receiver, Msg) ->
    ...
    ValidReceiver = dict:is_key(Receiver,
                                State#state.nstates),
    case NotDropped and ValidSender and ValidReceiver of
        false -> {fail, State};
        true -> handle_call(State, Sender, Receiver, Msg)
    end.

cast(State, undefined, _, _) -> {ok, State};
cast(State, _Node, undefined, _) -> {ok, State};
cast(State, Sender, Receiver, Msg) ->
    ...
    case NotDropped and ValidSender and ValidReceiver of
        false -> {ok, State};
        true -> {ok, handle_cast(State, Sender, Receiver, Msg)}
    end.
...
s_finish_handoff(State, AH = {Mod, Idx, Prev, New}) ->
    ...
    {_, NextOwner, Status} = next_owner(State,
                                        PrevCS1,
                                        Idx,
                                        Mod),
    ...
    case {Owner, NextOwner, NewStatus, Status} of
        ...
        {Prev, New, _, awaiting} ->
            ...
            PrevNS2 = update_partition_status(Mod,
                                              PrevNS1,
                                              Idx,
                                              true,
                                              false),
            PrevCS2 = mark_transfer_complete(State,
                                             PrevCS1,
                                             Idx,
                                             Mod),
            ...
            NewNS2 = update_partition_status(Mod,
                                             NewNS,
                                             Idx,
                                             true,
                                             true),
        ...
        {Prev, _, _, _} ->
            ...
            case lists:member(New, State#state.removed) of
                true -> ok;
                false -> throw("Why am I handing off?")
            end,
...
maybe_shutdown(State, Node) ->
    CState = get_cstate(State, Node),
    Ready = ring_ready(CState),
    Next = CState#chstate_v2.next,
    NoIndices = indices(CState, Node) =:= [],
    NoPartitions = count_vnodes(State, Node) =:= 0,
    NoPendingIndices = [] =:=
                           [Idx
                            || {Idx, _, NextOwner, _, _} <- Next,
                               NextOwner =:= Node],
    Shutdown = NoIndices and NoPartitions and
                   NoPendingIndices,
    Status = member_status(CState, Node),
...
reconcile(State, CS01, CS02) ->
    ...
    CS1 = CS03#chstate_v2{seen = Seen},
    CS2 = CS04#chstate_v2{seen = Seen},
    SeenChanged = not equal_seen(CS01, CS1),
    VC1 = CS1#chstate_v2.vclock,
    VC2 = CS2#chstate_v2.vclock,
    ...
    case {Equal, Newer, Older} of
        {_, true, false} ->
            ...
            {SeenChanged, CS1#chstate_v2{vclock = VC3}};
        {_, false, true} ->
            %%io:format("CS2: ~p~n", [CS2]),
            {true, CS2#chstate_v2{nodename = VNode, vclock = VC3}};
        {true, _, _} ->
            {SeenChanged, CS1#chstate_v2{vclock = VC3}};
        {_, true, true} ->
        ...
        {_, false, false} ->
            ...
            {true, CS3#chstate_v2{nodename = VNode}}
    end.

reconcile_divergent(State, VNode, CS1, CS2) ->
    VClock = vclock:increment(VNode,
                              vclock:merge([CS1#chstate_v2.vclock,
                                            CS2#chstate_v2.vclock])),
    Members = reconcile_members(CS1, CS2),
    CS3 = reconcile_ring(State,
                         CS1,
                         CS2,
                         get_members(Members)),
    CS3#chstate_v2{vclock = VClock, members = Members}.

reconcile_members(CS1, CS2) ->
    %%?debugFmt("M1: ~p~nM2: ~p~n", [CS1#chstate_v2.members, CS2#chstate_v2.members]),
    orddict:merge(fun (_K, {Valid1, VC1, _},
                       {Valid2, VC2, _}) ->
                          New1 = vclock:descends(VC1, VC2),
                          New2 = vclock:descends(VC2, VC1),
                          MergeVC = vclock:merge([VC1, VC2]),
                          case {New1, New2} of
                              {true, false} -> {Valid1, MergeVC, []};
                              {false, true} -> {Valid2, MergeVC, []};
                              {_, _} ->
                                  {merge_status(Valid1, Valid2), MergeVC, []}
                          end
                  end,
                  CS1#chstate_v2.members,
                  CS2#chstate_v2.members).
...
reconcile_ring(_State,
               CS1 = #chstate_v2{claimant = Claimant1, rvsn = VC1,
                                 next = Next1},
               CS2 = #chstate_v2{claimant = Claimant2, rvsn = VC2,
                                 next = Next2},
               Members) ->
...
update_partition_status(Mod, NState, Idx, Active,
                        HasData) ->
    Parts = NState#nstate.partitions,
    Data = NState#nstate.has_data,
    P = case orddict:find(Mod, Parts) of
            error -> [];
            {ok, P0} -> P0
        end,
    D = case orddict:find(Mod, Data) of
            error -> [];
            {ok, D0} -> D0
        end,
    ...
    NState#nstate{partitions =
                      orddict:store(Mod, P2, Parts),
...
count_vnodes(State, Node) ->
    NState = get_nstate(State, Node),
    length(lists:flatmap(fun ({_, Vs}) -> Vs end,
                         NState#nstate.partitions)).
...
vnode_running(State, Node, Mod, Idx) ->
    ...
    case orddict:find(Mod, NState#nstate.partitions) of
        error -> false;
        {ok, VNodes} -> lists:member(Idx, VNodes)
    end.
...
member_status(#chstate_v2{members = Members}, Node) ->
...
save_cstate(State, Node, CState) ->
    NS = dict:update(Node,
                     fun (NState) -> NState#nstate{chstate = CState} end,
                     #nstate{chstate = CState},
                     State#state.nstates),
...
ring_ready(CState0) ->
    ...
    Seen = CState#chstate_v2.seen,
    %% TODO: Should we add joining here?
    %%Members = get_members(CState#chstate_v2.members, [joining, valid, leaving, exiting]),
    Members = get_members(CState#chstate_v2.members,
                          [valid, leaving, exiting]),
    VClock = CState#chstate_v2.vclock,
    R = [begin
...
ring_changed(State, _RRing, {Node, _NState}, CState0) ->
    ...
    case ring_ready(CState) of
        false -> update_cstate(State, Node, CState);
        true ->
            {C1, State2, CState2} = maybe_update_claimant(State,
                                                          Node,
                                                          CState),
            {C2, State3, CState3} = maybe_handle_joining(State2,
                                                         Node,
                                                         CState2),
            case C2 of
                true ->
                ...
                false ->
                    {C3, State4, CState4} = maybe_update_ring(State3,
                                                              Node,
                                                              CState3),
                    {C4, State5, CState5} = maybe_remove_exiting(State4,
                                                                 Node,
                                                                 CState4),
                    case C1 or C2 or C3 or C4 of
                        ...
                        false -> update_cstate(State5, Node, CState5)
                    end
            end
    end.
...
maybe_update_claimant(State, Node, CState) ->
    Members = get_members(CState#chstate_v2.members,
                          [valid, leaving]),
    Claimant = CState#chstate_v2.claimant,
    RVsn = CState#chstate_v2.rvsn,
    ...
    case {ClaimantMissing, NextClaimant} of
        {true, Node} ->
            ...
            CState2 = CState#chstate_v2{claimant = Node,
                                        rvsn = RVsn2},
            {true, State, CState2};
        _ -> {false, State, CState}
    end.
    ...
    case Claimant of
        Node ->
            case claiming_members(CState) of
                ...
                _ ->
                    RRing = State#state.random_ring,
                    {Changed, State2, CState2} = update_ring(State,
                                                             RRing,
                                                             Node,
                                                             CState),
                    {Changed, State2, CState2}
...
maybe_remove_exiting(State, Node, CState) ->
    ...
    case Claimant of
        Node ->
            Exiting = get_members(CState#chstate_v2.members,
                                  [exiting])
                          -- [Node],
            %%io:format("Claimant ~p removing exiting ~p~n", [Node, Exiting]),
            Changed = Exiting /= [],
            {State2, CState2} = lists:foldl(fun (ENode,
                                                 {State0, CState0}) ->
                                                    {_, State02} = cast(State0,
                                                                        Node,
                                                                        ENode,
                                                                        shutdown),
                                                    CState02 = set_member(Node,
                                                                          CState0,
                                                                          ENode,
                                                                          invalid,
                                                                          same_vclock),
...
maybe_handle_joining(State, Node, CState) ->
    ...
    case Claimant of
        Node ->
            Joining = get_members(CState#chstate_v2.members,
                                  [joining]),
            %% ?debugFmt("Claimant ~p joining ~p~n", [Node, Joining]),
            Changed = Joining /= [],
            {State2, CState2} = lists:foldl(fun (JNode,
                                                 {State0, CState0}) ->
                                                    State02 = State0,
                                                    CState02 = set_member(Node,
                                                                          CState0,
...
update_ring(State, _RRing, CNode, CState) ->
    Next0 = CState#chstate_v2.next,
    ?ROUT("Members: ~p~n", [CState#chstate_v2.members]),
    ?ROUT("Updating ring :: next0 : ~p~n", [Next0]),
    %% Remove tuples from next for removed nodes
    InvalidMembers = get_members(CState#chstate_v2.members,
                                 [invalid]),
    Next2 = lists:filter(fun (NInfo) ->
                                 {Owner, NextOwner, _} = next_owner(State,
                                                                    NInfo),
                                 not lists:member(Owner, InvalidMembers) and
                                     not lists:member(NextOwner, InvalidMembers)
                         end,
                         Next0),
    CState2 = CState#chstate_v2{next = Next2},
    %% Transfer ownership after completed handoff
    {RingChanged1, CState3} = transfer_ownership(State,
                                                 CState2),
    ?ROUT("Updating ring :: next1 : ~p~n",
          [CState3#chstate_v2.next]),
    %% Ressign leaving/inactive indices
    {RingChanged2, State2, CState4} =
        reassign_indices(State, CState3),
    ?ROUT("Updating ring :: next2 : ~p~n",
          [CState4#chstate_v2.next]),
    ...
    case Changed of
        true ->
            RVsn2 = vclock:increment(CNode,
                                     CState4#chstate_v2.rvsn),
            ?ROUT("Updating ring :: next3 : ~p~n", [Next4]),
            {true,
             State2,
             CState4#chstate_v2{next = Next4, rvsn = RVsn2}};
        false -> {false, State, CState}
    end.

transfer_ownership(State,
                   CState = #chstate_v2{next = Next}) ->
    ...
    {Changed, CState2#chstate_v2{next = Next2}}.

reassign_indices(State,
                 CState = #chstate_v2{next = Next}) ->
    Invalid = get_members(CState#chstate_v2.members,
                          [invalid]),
    {State2, CState2} = lists:foldl(fun (Node,
                                         {State0, Ring0}) ->
                                            Allowed2 = State0#state.allowed ++
                                                           indices(Ring0, Node),
                                            Ring2 = remove_node(Ring0,
                                                                Node,
                                                                invalid),
                                            {State0#state{allowed = Allowed2},
                                             Ring2}
                                    end,
                                    {State, CState},
                                    Invalid),
    CState3 = case Next of
                  [] ->
                      Leaving = get_members(CState#chstate_v2.members,
                                            [leaving]),
    ...
    NextChanged = Next /= CState3#chstate_v2.next,
    {RingChanged or NextChanged, State2, CState3}.

rebalance_ring(_CNode,
               CState = #chstate_v2{next = []}) ->
    ...
    Next;
rebalance_ring(_CNode,
               _CState = #chstate_v2{next = Next}) ->
    Next.

handle_down_nodes(CState, Next) ->
    LeavingMembers = get_members(CState#chstate_v2.members,
                                 [leaving, invalid]),
    DownMembers = get_members(CState#chstate_v2.members,
                              [down]),
    Next2 = [begin
...
all_next_owners(#chstate_v2{next = Next}) ->
...
remove_node(CState, _Node, _Status, []) -> CState;
remove_node(CState, Node, Status, Indices) ->
    %% ?debugFmt("Reassigning from ~p: ~p~n", [Node, indices(CState, Node)]),
    CStateT1 = change_owners(CState,
...
remove_from_cluster(Ring, ExitingNode) ->
    ...
    ExitRing = case attempt_simple_transfer(Ring,
                                            AllOwners,
                                            ExitingNode)
                   of
                   {ok, NR} -> NR;
...
attempt_simple_transfer(Ring, Owners, ExitingNode) ->
    ...
    TargetN = 3,
    attempt_simple_transfer(Ring,
                            Owners,
                            TargetN,
                            ExitingNode,
                            0,
                            [{O, -TargetN}
                             || O <- claiming_members(Ring), O /= ExitingNode]).

attempt_simple_transfer(Ring, [{P, Exit} | Rest],
        ...
        Candidates ->
            ...
            case lists:filter(fun (N) ->
                ...
                Qualifiers ->
                    ...
                    attempt_simple_transfer(riak_core_ring:transfer_node(P,
                                                                         Chosen,
                                                                         Ring),
                                            Rest,
                                            TargetN,
...
attempt_simple_transfer(Ring, [{_, N} | Rest], TargetN,
                        Exit, Idx, Last) ->
...
update_members(State = #state{members = Members},
               CS2) ->
    CS1 = #chstate_v2{members = Members},
...
increment_cstate_vclock(Node, CState) ->
    VClock = vclock:increment(Node,
                              CState#chstate_v2.vclock),
    CState#chstate_v2{vclock = VClock}.

%% Returns true if the nodes can communicate (ie. not split)
can_communicate(_State, N1, N2) when N1 =:= N2 -> true;
can_communicate(State, N1, N2) ->
    ...
    dict:is_key(N1, State#state.nstates) and
        dict:is_key(N2, State#state.nstates)
        and not dict:is_key({N1, N2}, State#state.split).

next_owner(State, CState, Idx) ->
    case lists:keyfind(Idx, 1, CState#chstate_v2.next) of
        false -> {undefined, undefined, undefined};
        NInfo -> next_owner(State, NInfo)
    end.
...
next_owner(_State,
           {_, Owner, NextOwner, _Transfers, Status}) ->
    {Owner, NextOwner, Status}.

mark_transfer_complete(State,
                       CState = #chstate_v2{next = Next, vclock = VClock}, Idx,
                       Mod) ->
    ...
    CState#chstate_v2{next = Next2, vclock = VClock2}.
...
equal_cstate(CS1, CS2) ->
    T1 = equal_members(CS1#chstate_v2.members,
                       CS2#chstate_v2.members),
    T2 = equal_vclock(CS1#chstate_v2.rvsn,
                      CS2#chstate_v2.rvsn),
    T3 = equal_seen(CS1, CS2),
    %% Clear fields checked manually and test remaining through equality.
    CS3 = CS1#chstate_v2{nodename = ok, members = ok,
                         vclock = ok, rvsn = ok, seen = ok},
    CS4 = CS2#chstate_v2{nodename = ok, members = ok,
                         vclock = ok, rvsn = ok, seen = ok},
    T4 = CS3 =:= CS4,
    T1 and T2 and T3 and T4.

equal_members(M1, M2) ->
    L = orddict:merge(fun (_, {Status1, VC1, _},
                           {Status2, VC2, _}) ->
                              Status1 =:= Status2 andalso equal_vclock(VC1, VC2)
                      end,
                      M1,
                      M2),
    {_, R} = lists:unzip(L),
...
filtered_seen(CS = #chstate_v2{seen = Seen}) ->
    case get_members(CS#chstate_v2.members) of
        [] -> Seen;
        Members ->
...
check_read(State, Owner, Idx) ->
    ...
    case MayFail or OwnerDown of
        true -> true;
        false -> check_read2(State, Owner, Idx)
    end.
            ...
            case next_owner(State, OwnerCS, Idx, riak_kv) of
                {_, Node, complete} ->
                    ...
                    case can_communicate(State, Owner, Node) of
                        true -> check_read(State, Node, Idx);
                        false ->
...
run_cmd(S, {_, _, Cmd}) -> run(S, Cmd).

run_cmds(Cmds) -> run_cmds(?RING, Cmds).

run_cmds(RingSize, Cmds) ->
    S0 = (initial_state())#state{ring_size = RingSize},
    State = lists:foldl(fun ({_, _, Cmd}, S) -> run(S, Cmd)
                        end,
                        S0,
                        Cmds),
    ?debugFmt("------------------------------~n", []),
    %% Most update to data active member
    {_, Member0} = lists:foldl(fun (Node,
                                    {VClock0, Result}) ->
                                       CS = get_cstate(State, Node),
                                       VClock1 = CS#chstate_v2.vclock,
                                       case vclock:descends(VClock1, VClock0) of
                                           true -> {VClock1, Node};
                                           false -> {VClock0, Result}
                                       end
                               end,
                               {vclock:fresh(), undefined},
                               State#state.primary),
    ...
    ?debugFmt("Owners1: ~p~nOwners2: ~p~n",
              [Owners1, Owners2]),
    ?debugFmt("Next0: ~p~n", [CState#chstate_v2.next]),
    ?debugFmt("Next1: ~p~n", [CState2#chstate_v2.next]),
    State4.

%% TODO: Re-add manual tests.
manual_test_list() -> [fun test_down_reassign/0].

test_down_reassign() ->
    run_cmds([{set,
               {var, 1},
               {call,
                new_cluster_membership_model_eqc,
                initial_cluster,
                [{[2, 3, 5], [0, 1, 4]},
                 [0, 1, 2, 3, 4, 5, 6, 7],
                 {1, 1, 1}]}},
              {set,
               {var, 2},
               {call, new_cluster_membership_model_eqc, down, [2, 3]}},
              {set,
               {var, 4},
               {call, new_cluster_membership_model_eqc, leave, [3]}},
              {set,
               {var, 5},
               {call, new_cluster_membership_model_eqc, join, [1, 5]}},
              {set,
               {var, 6},
               {call, new_cluster_membership_model_eqc, join, [0, 3]}},
              {set,
               {var, 9},
               {call,
                new_cluster_membership_model_eqc,
