...
handle_work(die, _From, _State) -> exit(test_die);
handle_work(Work, _From, State) -> {reply, Work, State}.
    ...
    ?FORALL({Seed, ExtraWork, WorkList},
            ...
            (aggregate([{{extra_work,
                       ...
                       ?WHENFAIL((io:format(user,
                                            "PoolSize: ~b~nWorkList: ~p~nSchedule: "
                                            "~p~n",
                                 ...
                                 begin
                                     ...
                                     true ==
                                         all_work_gets_done(Seed,
                                                            PoolSize,
                                                            WorkList)
                                 end)))).

pool_size(false, WorkList) -> length(WorkList);
...
all_work_gets_done(PoolSize, WorkList) ->
    ...
    [riak_core_vnode_worker_pool:handle_work(Pool,
                                             ...
                                             {raw, N, self()})
     || {N, W}
            <- lists:zip(lists:seq(1, length(WorkList)), WorkList)],
