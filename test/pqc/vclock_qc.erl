...
proper_test_() ->
    {timeout,
     120,
     ?_assert((proper:quickcheck(prop_vclock(),
                                 [{numtests, 5000}])))}.
                          ...
                          [{call,
                           ...
                           || length(Vs) > 0]
                              ...
                              [{call,
                               ...
                               || length(Vs) > 0]
                                  ++
                                  [{call, ?MODULE, merge, [list(elements(Vs))]}
                                   || length(Vs) > 0]
                                      ++
                                      [{call,
                                        ?MODULE,
...
next_state(S, _V, {call, _, get_counter, [_, _]}) -> S;
next_state(S, _V, {call, _, get_timestamp, [_, _]}) ->
    S;
next_state(S, _V, {call, _, get_dot, [_, _]}) -> S;
next_state(S, _V, {call, _, descends, [_, _]}) -> S;
next_state(S, _V, {call, _, descends_dot, [_, _, _]}) ->
    S;
next_state(S, _V, {call, _, dominates, [_, _]}) -> S;
next_state(S, _V, {call, _, timestamp, _}) -> S;
next_state(#state{vclocks = Vs} = S, V,
           {call, _, _, _}) ->
...
postcondition(_S, {call, _, get_counter, _},
              {MRes, Res}) ->
    MRes == Res;
postcondition(_S, {call, _, get_timestamp, _},
              {MRes, Res}) ->
    timestamp_values_equal(MRes, Res);
postcondition(_S, {call, _, get_dot, _},
              {{_Actor, 0, 0}, undefined}) ->
    true;
postcondition(_S, {call, _, get_dot, _},
              {{Actor, Cnt, TS}, {ok, {Actor, {Cnt, TS}}}}) ->
    true;
postcondition(_S, {call, _, get_dot, _}, {_, _}) ->
    false;
postcondition(_S, {call, _, descends, _},
              {MRes, Res}) ->
    MRes == Res;
postcondition(_S, {call, _, descends_dot, _},
              {MRes, Res}) ->
    MRes == Res;
postcondition(_S, {call, _, dominates, _},
              {MRes, Res}) ->
    MRes == Res;
postcondition(_S, {call, _, merge, [Items]},
              {MRes, Res}) ->
    model_compare(MRes, Res) andalso
        lists:all(fun ({_, V}) -> vclock:descends(Res, V) end,
                  Items);
postcondition(_S, _C, {MRes, Res}) ->
    model_compare(MRes, Res);
postcondition(_S, _C, _Res) -> true.
            ...
            begin
                ...
                aggregate([length(V) || {_, V} <- S#state.vclocks],
                          aggregate(command_names(Cmds),
                                    collect({num_vclocks_div_10,
                                             length(S#state.vclocks) div 10},
                                            Res == ok)))
            end).

gen_actor_id() -> elements(?ACTOR_IDS).

gen_entry(Actor, S) ->
    ?LET({_M, V}, (elements(S)),
         (vclock:get_dot(Actor, V))).

fresh() -> {new_model(), vclock:fresh()}.

dominates({AM, AV}, {BM, BV}) ->
    {model_descends(AM, BM) andalso
         not model_descends(BM, AM),
...
increment(A, {M, V}) ->
    ...
    {lists:keyreplace(A, 1, M, {A, MCount + 1, TS}),
     vclock:increment(A, TS, V)}.
...
model_descends(AM, BM) ->
    lists:all(fun ({{Actor, Count1, _TS1},
                    {Actor, Count2, _TS2}}) ->
                      Count1 >= Count2
              end,
              lists:zip(AM, BM)).
...
timestamp() -> put(timestamp, get(timestamp) + 1).
...
model_entry(A, M) -> lists:keyfind(A, 1, M).
...
timestamp_values_equal(0, undefined) -> true;
timestamp_values_equal(T, T) -> true;
timestamp_values_equal(_, _) -> false.
