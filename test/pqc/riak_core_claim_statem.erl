...
claim_test_() ->
    {timeout,
     120,
     ?_assert((proper:quickcheck(prop_claim(with_ring_size(5)),
                                 [{numtests, 5000}])))}.
...
-record(state,
        {ring_size,
         nodes = [?CLAIMANT] ::
             [atom()], %% nodes that have been added
         node_counter = 1 ::
             non_neg_integer(), %% to aid with naming nodes
         ring = undefined,
         committed_nodes = []}).

%% @doc run the statem with a ring of size `math:pow(`N', 2)'.
-spec
     with_ring_size(pos_integer()) -> proper_statem:symbolic_state().

with_ring_size(N) ->
    RingSize = trunc(math:pow(2, N)),
    #state{ring_size = RingSize,
           ring = riak_core_ring:fresh(RingSize, ?CLAIMANT)}.

%% @doc Returns the state in which each test case starts. (Unless a different
%%      initial state is supplied explicitly to, e.g. commands/2.)
-spec
     initial_state_data() -> proper_statem:symbolic_state().

initial_state_data() -> #state{}.

initial_state(_S) -> starting.

starting(S) ->
    [{planning,
      {call, ?MODULE, add_node, add_node_args(S)}}].

planning(S) ->
    [{planning,
      {call, ?MODULE, add_node, add_node_args(S)}},
     {planning,
      {call, ?MODULE, leave_node, leave_node_args(S)}}].
...
precondition(_F, _T, S = #state{nodes = Nodes},
             {call, _, add_node, _})
    when S#state.ring_size div length(Nodes) =< 3 ->
    false;
...
precondition(_F, _T, #state{nodes = Nodes},
             {call, _, leave_node, _})
    when length(Nodes) < 5 ->
    false;
...
precondition(_F, _T, #state{ring = undefined},
             {call, _, claim, _}) ->
    false;
%claim_pre(_From, _To, _S) ->
precondition(_F, _T, _S, {call, _, claim, _}) -> true.
...
next_state_data(_F, _T,
                S = #state{node_counter = NC, nodes = Nodes}, Ring,
                {call, _, add_node, [Node, _RingIn]}) ->
    S#state{ring = Ring, node_counter = NC + 1,
            nodes = [Node | Nodes]};
...
%          NewS :: proper:symbolic_state() | proper:dynamic_state().
next_state_data(_F, _T,
                S = #state{committed_nodes = Committed, nodes = Nodes},
                Ring, {call, _, leave_node, [Node, _RingIn]}) ->
    S#state{ring = Ring,
            committed_nodes = lists:delete(Node, Committed),
...
next_state_data(_F, _T, S = #state{nodes = Nodes},
                NewRing, {call, _, claim, [_OldRing]}) ->
...
postcondition(_F, _T, _S,
              {call, _, add_node, [NodeName, _Ring]}, NextRing) ->
    lists:member(NodeName,
                 riak_core_ring:members(NextRing, [joining]));
...
%          Res  :: term().
postcondition(_F, _T, _S,
              {call, _, leave_node, [NodeName, _Ring]}, NextRing) ->
    lists:member(NodeName,
                 riak_core_ring:members(NextRing, [leaving]));
...
postcondition(_F, _T, #state{nodes = Nodes},
              {call, _, claim, [_Ring]}, _NewRing)
    when length(Nodes) < 4 ->
    true;
%claim_post(_From, _To, _S, [_Ring], NewRing) ->
postcondition(_F, _T, _S, {call, _, claim, [_Ring]},
              NewRing) ->
    Nval = 3,
    TNval = 4,
...
add_node(NodeName, Ring) ->
    R = riak_core_ring:add_member(?CLAIMANT,
                                  Ring,
                                  NodeName),
    R.
...
leave_node(NodeName, Ring) ->
    R = riak_core_ring:leave_member(?CLAIMANT,
                                    Ring,
                                    NodeName),
    R.
...
claim_args(#state{ring = Ring}) -> [Ring].

%% @doc claim - The actual operation
claim(Ring) ->
    R = riak_core_claim:claim(Ring,
...
-spec
     prop_claim(proper_statem:symbolic_state()) -> proper:property().
...
ring_size(undefined) -> 0;
ring_size(Ring) -> riak_core_ring:num_partitions(Ring).

node_count(undefined) -> 0;
node_count(Ring) ->
    length(riak_core_ring:members(Ring, [joining, valid])).

weight(_From, _To, add_node, _Args) -> 5;
weight(_, _, _, _) -> 1.
...
transfer_ring(Ring) ->
    ...
    RFinal = lists:foldl(fun ({Idx, Owner}, Racc) ->
                                 riak_core_ring:transfer_node(Idx, Owner, Racc)
                         end,
                         Ring,
                         Owners),
    RFinal.
