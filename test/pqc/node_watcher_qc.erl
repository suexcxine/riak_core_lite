...
-define(ORDSET(L), ordsets:from_list(L)).%TODO
                                         % qc_test_() ->
                                         %     {timeout, 1500,
    ...
    ?SETUP(fun setup_cleanup/0,
           (?FORALL(Cmds, (commands(?MODULE)),
                    begin
                        ...
                        riak_core_node_watcher:set_broadcast_module(?MODULE,
                                                                    on_broadcast),
                        %% Run the test
...
setup_cleanup() ->
    ...
    RingEventHandlerSup =
        ensure_started(riak_core_eventhandler_sup),
    RingEvents = ensure_started(riak_core_ring_events),
    NodeWatcherEvents =
        ensure_started(riak_core_node_watcher_events),
...
initial_state() -> #state{up_nodes = [node()]}.

command(S) ->
    oneof([{call, ?MODULE, ring_update, [g_ring_nodes()]},
...
precondition(S,
             {call, _, local_service_kill, [Service, S]}) ->
    orddict:is_key(Service, S#state.service_pids);
precondition(S, {call, _, wait_for_bcast, _}) ->
    is_node_up(node(), S);
precondition(S, {call, _, Test, [Service, S]})
    when Test =:= health_service_up;
         Test =:= health_service_down;
         Test =:= health_service_error ->
    is_node_up(node(), S) andalso
        lists:member(Service, S#state.service_healths);
precondition(_, _) -> true.

next_state(S, Res,
           {call, _, local_service_up, [Service]}) ->
    S2 = service_up(node(), Service, S),
    Pids = orddict:store(Service,
                         Res,
                         S2#state.service_pids),
    Healths = orddict:erase(Service,
                            S2#state.service_healths),
    S2#state{service_pids = Pids,
             service_healths = Healths};
next_state(S, _Res,
           {call, _, local_service_down, [Service]}) ->
    ...
    S2#state{service_pids = Pids,
             service_healths = Healths};
next_state(S, _Res,
           {call, _, local_service_kill, [Service, _]}) ->
    S2 = service_down(node(), Service, S),
    Pids = orddict:erase(Service, S2#state.service_pids),
    Healths = orddict:erase(Service,
                            S2#state.service_healths),
...
next_state(S, _Res, {call, _, local_node_down, []}) ->
    node_down(node(), S);
next_state(S, _Res,
           {call, _, remote_service_up, [Node, Services]}) ->
    peer_up(Node, Services, S);
next_state(S, _Res, {call, _, Fn, [Node]})
    when Fn == remote_service_down;
         Fn == remote_service_down_disterl ->
    peer_down(Node, S);
next_state(S, _Res, {call, _, wait_for_bcast, _}) -> S;
next_state(S, Res, {call, _, HPService, [Service]})
    when HPService =:= health_service;
         HPService =:= health_service_defaults ->
    S2 = service_up(node(), Service, S),
    Pids = orddict:store(Service,
                         Res,
                         S2#state.service_pids),
    Healths = orddict:store(Service,
                            Res,
                            S2#state.service_healths),
    S2#state{service_pids = Pids,
             service_healths = Healths};
...
next_state(S, _Res,
           {call, _, health_service_up, [Service, _]}) ->
    S2 = service_up(node(), Service, S),
    ...
    S2#state{service_pids = Pids};
next_state(S, _Res,
           {call, _, health_service_down, [Service, _]}) ->
    S2 = service_down(node(), Service, S),
    Pids = orddict:erase(Service, S2#state.service_pids),
    S2#state{service_pids = Pids};
next_state(S, _Res,
           {call, _, health_service_error, [Service, _]}) ->
    S2 = service_down(node(), Service, S),
    Pids = orddict:erase(Service, S2#state.service_pids),
    S2#state{service_pids = Pids};
next_state(S, _Res, {call, _, ring_update, [Nodes]}) ->
    Peers = ordsets:del_element(node(),
                                ordsets:from_list(Nodes)),
    peer_filter(S#state{peers = Peers}).

postcondition(S, {call, _, local_service_up, [Service]},
              _Res) ->
    S2 = service_up(node(), Service, S),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
postcondition(S,
              {call, _, local_service_down, [Service]}, _Res) ->
    S2 = service_down(node(), Service, S),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
postcondition(S,
              {call, _, local_service_kill, [Service, _]}, _Res) ->
    S2 = service_down(node(), Service, S),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
...
postcondition(S, {call, _, local_node_down, _}, _Res) ->
    ...
    deep_validate(S2);
postcondition(S,
              {call, _, remote_service_up, [Node, Services]}, _Res) ->
        ...
        true ->
            case is_node_up(node(), S) of
                true ->
                    ...
                    ?assertEqual({{up, node(), ExpServices}, [Node]},
                                 (hd(Bcasts)));
                false ->
                    ?assertEqual({{down, node()}, [Node]}, (hd(Bcasts)))
            end;
        false -> ?assertEqual([], Bcasts)
    end,
...
postcondition(S, {call, _, Fn, [Node]}, _Res)
    ...
    S2 = case is_peer(Node, S) of
             true -> node_down(Node, S);
             false -> S
         end,
    deep_validate(S2);
postcondition(S, {call, _, wait_for_bcast, _}, _Res) ->
    validate_broadcast(S, S, service);
postcondition(S, {call, _, health_service, [Service]},
              _Res) ->
    S2 = service_up(node(), Service, S),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
postcondition(S,
              {call, _, health_service_defaults, [Service]}, _Res) ->
    S2 = service_up(node(), Service, S),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
postcondition(S,
              {call, _, health_service_up, [Service, _]}, _Res) ->
    case is_service_up(node(), Service, S) of
        true -> deep_validate(S);
        false ->
            S2 = service_up(node(), Service, S),
...
postcondition(S,
              {call, _, health_service_down, [Service, _]}, _Res) ->
    S2 = service_down(node(), Service, S),
    ?assert((meck:validate(mod_health))),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
postcondition(S,
              {call, _, health_service_error, [Service, _]}, _Res) ->
    S2 = service_down(node(), Service, S),
    ?assert((meck:validate(mod_health))),
    validate_broadcast(S, S2, service),
    deep_validate(S2);
postcondition(S, {call, _, ring_update, [Nodes]},
              _Res) ->
    ...
    case is_node_up(node(), S) of
        true ->
            ExpServices = services(node(), S),
            ?assertEqual({{up, node(), ExpServices}, NewPeers},
                         (hd(Bcasts)));
        false ->
...
deep_validate(S) ->
    ...
    %% service.
    ExpNodes = (?ORDSET([snodes(Svc, S)
                         || Svc <- ExpAllServices])),
    ActNodes =
        (?ORDSET([?ORDSET((riak_core_node_watcher:nodes(Svc)))
                  || Svc <- ExpAllServices])),
...
validate_broadcast(S0, Sfinal, Op, Bcasts) ->
    Transition = {is_node_up(node(), S0),
                  is_node_up(node(), Sfinal),
                  Op},
    ExpPeers = Sfinal#state.peers,
    case Transition of
        {false, true, _} -> %% down -> up
            ExpServices = services(node(), Sfinal),
            ?assertEqual({{up, node(), ExpServices}, ExpPeers},
                         (hd(Bcasts)));
...
g_service() -> oneof([s1, s2, s3, s4]).

g_node() ->
    oneof(['n1@127.0.0.1', 'n2@127.0.0.1', 'n3@127.0.0.1']).

g_services() -> list(elements([s1, s2, s3, s4])).

g_ring_nodes() ->
    vector(application:get_env(riak_core,
                               ring_creation_size,
                               undefined),
           oneof([node(),
                  'n1@127.0.0.1',
...
local_node_up() -> riak_core_node_watcher:node_up().

local_node_down() -> riak_core_node_watcher:node_down().

remote_service_up(Node, Services) ->
    Avsn0 = riak_core_node_watcher:avsn(),
    gen_server:cast(riak_core_node_watcher,
                    {up, Node, Services}),
    wait_for_avsn(Avsn0).
...
health_service(Service) ->
    Avsn0 = riak_core_node_watcher:avsn(),
    Pid = spawn(fun () -> service_loop() end),
    ok = riak_core_node_watcher:service_up(Service,
                                           Pid,
                                           {mod_health, callback, [Pid]},
                                           [{max_callback_failures, 1},
...
health_service_up(Service, S) ->
    ...
    receive
        meck_done -> ok after 100 -> erlang:error(timeout)
    end,
    case is_service_up(node(), Service, S) of
        true -> ok;
...
node_up(Node, S) ->
    S#state{up_nodes =
                ordsets:add_element(Node, S#state.up_nodes)}.

node_down(Node, S) ->
    S#state{up_nodes =
                ordsets:del_element(Node, S#state.up_nodes)}.

service_up(Node, Service, S) ->
    S#state{services =
                ordsets:add_element({Node, Service}, S#state.services)}.

services_up(Node, Services, S) ->
    NewServices = (?ORDSET([{Node, Svc}
                            || Svc <- Services])),
    OldServices = [{N, Svc}
                   || {N, Svc} <- S#state.services, Node /= N],
    S#state{services =
                ordsets:union(NewServices, OldServices)}.

service_down(Node, Svc, S) ->
    S#state{services =
                ordsets:del_element({Node, Svc}, S#state.services)}.
...
peer_up(Node, Services, S) ->
    case is_peer(Node, S) of
        true -> node_up(Node, services_up(Node, Services, S));
        false -> S
    end.
...
services(Node, S) ->
    case ordsets:is_element(Node, S#state.up_nodes) of
        true -> all_services(Node, S);
        false -> []
    end.

snodes(S) -> S#state.up_nodes.

snodes(Service, S) ->
    ?ORDSET([Node
             || {Node, Svc} <- S#state.services,
                ordsets:is_element(Node, S#state.up_nodes),
                Svc == Service]).
...
broadcasts() ->
    Bcasts = [list_to_tuple(L)
              || L <- ets:match(?MODULE, {'_', '$1', '$2'})],
