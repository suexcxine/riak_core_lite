     ...
     [{spawn,
       [{setup,
         ...
         [{timeout,
           ...
           ?_assertEqual(true,
                         (proper:quickcheck(?QC_OUT((prop_chash_next_index())),
                                            [{numtests,
                                              ?TEST_ITERATIONS}])))}]}]}]}.
...
g_partition_exponent() -> choose(1, 12).
