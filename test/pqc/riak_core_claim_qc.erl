...
property_claim_ensures_unique_nodes_v2_test_() ->
    ...
    {timeout,
     120,
     fun () ->
             ?_assert((proper:quickcheck(Prop, [{numtests, 5000}])))
     end}.
    ...
    ?FORALL({PartsPow, NodeCount},
            ...
            begin
                ...
                RAdded = lists:foldl(fun (Node, Racc) ->
                                     ...
                                     R0,
                                     RestNodes),
                Rfinal = riak_core_claim:claim(RAdded,
                                               {?MODULE, wants_claim_v2},
                                               {?MODULE, ChooseFun}),
                ...
                ?WHENFAIL(begin
                          ...
                          (conjunction([{meets_target_n,
                                         equals({true, []},
                                                riak_core_claim:meets_target_n(Rfinal,
                                                                               TNval))},
                                        {perfect_preflists,
                                         equals([], ImperfectPLs)},
                                        {balanced_ring,
                                         balanced_ring(Partitions,
                                                       NodeCount,
                                                       Rfinal)}])))
            end).
    ...
    ?FORALL({PartsPow, BaseNodes, AddedNodes},
            ...
            begin
                ...
                RBase = lists:foldl(fun (Node, Racc) ->
                                    ...
                                    R0,
                                    RestNodes),
                Rinterim = riak_core_claim:claim(RBase,
                                                 {?MODULE, wants_claim_v2},
                                                 {?MODULE, ChooseFun}),
                RAdded = lists:foldl(fun (Node, Racc) ->
                                             riak_core_ring:add_member(Node0,
...
balanced_ring(RingSize, NodeCount, Ring) ->
    TargetClaim = riak_core_claim:ceiling(RingSize /
                                              NodeCount),
...
wants_counts_test() ->
    {timeout,
     120,
     ?assert((proper:quickcheck(?QC_OUT((prop_wants_counts())),
                                [{numtests, 5000}])))}.
...
wants_test() ->
    {timeout,
     120,
     ?_assert((proper:quickcheck(?QC_OUT((prop_wants())),
                                 [{numtests, 5000}])))}.
    ...
    ?FORALL({NodeStatus, Q},
            ...
            begin
                ...
                ?WHENFAIL(begin
                          ...
                          (conjunction([{wants,
                                         length(Wants) == length(NodeStatus)},
                                        {active, Q == ActiveSum},
