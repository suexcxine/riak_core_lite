...
prop_future_index() ->
    ?FORALL(({CHashKey, OrigIdx, TargetIdx, N, Pos, Ring} =
                 TransferItem),
            (resize_item()),
            (?WHENFAIL((prop_future_index_failed(TransferItem)),
                       (collect(N =/= undefined andalso Pos >= N,
                                begin
                                    {Time, Val} = timer:tc(riak_core_ring,
                                                           future_index,
                                                           [CHashKey,
                                                            OrigIdx,
                                                            N,
                                                            Ring]),
                                    measure(future_index_usec,
                                            Time,
                                            TargetIdx =:= Val)
                                end))))).
    ...
    ?LET({RingSize, GrowthF},
         ...
         (?LET({IndexStart, Pos, N},
               ...
               begin
                   ...
                   Ring1 = riak_core_ring:resize(Ring0,
                                                 trunc(GrowthF * RingSize)),
                   ...
                   FuturePreflist = riak_core_ring:preflist(CHashKey,
                                                            Ring1),
                   ...
                   case Pos >= riak_core_ring:num_partitions(Ring1) orelse
                            N =/= undefined andalso Pos >= N
                       of
                       true -> TargetIdx = undefined;
                       false ->
                           {TargetIdx, _} = lists:nth(Pos + 1, FuturePreflist)
                   end,
...
index_in_current_ring(RingSize) ->
    elements([I
              || {I, _}
                     <- riak_core_ring:all_owners(riak_core_ring:fresh(RingSize,
                                                                       node()))]).

current_size() -> elements([16, 32, 64, 128]).

growth_factor() -> oneof([0.25, 0.5, 2, 4]).
...
prop_future_index_failed({CHashKey,
                          OrigIdx,
                          TargetIdx,
                          NValCheck,
                          _,
    ...
    FoundTarget = riak_core_ring:future_index(CHashKey,
                                              OrigIdx,
                                              NValCheck,
                                              R),
    io:format("key: ~p~nsource: ~p~ncurrsize: ~p~nfuturesize"
              ": ~p~nexpected: ~p~nactual: ~p~n",
              [CHashInt,
               OrigIdx,
               riak_core_ring:num_partitions(R),
               riak_core_ring:future_num_partitions(R),
               TargetIdx,
               FoundTarget]).
