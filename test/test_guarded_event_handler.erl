...
wait_for_exitfun() ->
    receive
        {?MODULE, {'EXIT', crash}} -> ok after 5000 -> fail
    end.

guarded_handler_test_case() ->
    Self = self(),
    F = fun (Handler, Reason) -> Self ! {Handler, Reason}
        end,
    ...
    ?assertEqual((riak_core:delete_guarded_event_handler(?MODULE,
                                                         ?MODULE,
                                                         quux)),
                 quux),
    ?assertNot((lists:member(?MODULE,
                             gen_event:which_handlers(?MODULE)))),
    ?assertEqual([],
                 (supervisor:which_children(riak_core_eventhandler_sup))).

wait_for_handler(_, 0, _) -> fail;
