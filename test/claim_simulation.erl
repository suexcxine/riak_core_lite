...
-define(node(N),
        list_to_atom(lists:flatten(io_lib:format("riak@node~w",
                                                 [N])))).
...
basic_test_() ->
    {timeout,
     60000,
     [fun basic_default/0, fun basic_new/0]}.
...
run(Opts) ->
    ...
    {Rings, _} = lists:mapfoldl(fun (N, Prev) ->
                                        Node = (?node(N)),
                                        R = riak_core_ring:add_member(Node,
                                                                      Prev,
                                                                      Node),
                                        Next =
                                ...
                                Ring1,
                                lists:seq(2, NodeCount)),
    Owners1 = riak_core_ring:all_owners(Ring1),
    Owners = lists:map(fun riak_core_ring:all_owners/1,
                       Rings),
    {Movers, _} = lists:mapfoldl(fun (Curr, Prev) ->
                                         Sum =
                                             length(lists:filter(fun not_same_node/1,
                                                                 lists:zip(Prev,
                                                                           Curr))),
                                         {Sum, Curr}
                                 end,
                                 Owners1,
                                 Owners),
    MeetTargetN = [riak_core_claim:meets_target_n(R,
                                                  TargetN)
                   || R <- Rings],
    ...
    [print_info(Out,
                ...
                N,
                M,
                lists:nth(N - 1, MeetTargetN),
                RingSize,
                NodeCapacity)
     || {O, M, N}
            <- lists:zip3(Owners, Movers, lists:seq(2, NodeCount))],
...
print_info(Out, Owners, NodeCount, PartitionsMoved,
           MeetsTargetN, RingSize, NodeCapacity) ->
    Expect = round(RingSize / NodeCount),
    ActualPercent = 100 * (PartitionsMoved / RingSize),
    Terabytes = 0.8 * NodeCount * NodeCapacity *
                    (PartitionsMoved / RingSize),
    %% Terabytes = Total Amount of Data * Fraction Moved
    F = fun ({_, Node}, Acc) ->
                dict:update_counter(Node, 1, Acc)
        end,
    C = lists:keysort(1,
                      dict:to_list(lists:foldl(F, dict:new(), Owners))),
    io:format(Out,
              "add node ~p, expect=~p, actual=~p, percent=~p"
              ", terabytes=~p, meets_target_n=~p~ncounts: "
              "~p~n~n",
              [NodeCount,
               Expect,
               PartitionsMoved,
